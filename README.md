...
- `letterboxd-scraper enrich [--tmdb/--no-tmdb] [--histograms/--no-histograms] [--limit N] [--slug film-slug] [--workers N] [--tmdb-rps X] [--force]` — second pass that hydrates touched films with TMDB metadata (IDs, runtime, poster, directors) and Letterboxd histogram stats from `/csi/film/{slug}/ratings-summary/`. Workers default to `scraper.max_concurrency` from your config, and TMDB requests are throttled globally (default `--tmdb-rps 10`). `--force` re-enriches even when metadata already exists. Use `--slug` when only one film needs an enrichment retry.
- `letterboxd-scraper film ids <slug>` — fetch the live Letterboxd film page and print the TMDB/IMDb identifiers found in the link buttons. Helpful when debugging enrichment issues.
- `letterboxd-scraper film sync-ids <slug> [--apply]` — scrape the TMDB/IMDb IDs for a film and optionally write them back to the database (pass `--apply` to persist). This also refreshes the slug/title when Letterboxd renames a film, so enrichment keeps tracking the right record.
